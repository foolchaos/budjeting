...
В разделе «Справочники» → «Договор» доступна кнопка **Импорт**. Поддерживаются файлы формата `.xlsx`
с ограничениями, аналогичными остальным справочникам: размер не более 10 МБ, максимум 50 000 строк данных,
обрабатывается только первый лист книги.

Строка заголовка должна содержать шесть столбцов: «Наименование», «№ внутренний», «№ внешний», «Дата», «Ответственный»,
«Контрагент». Столбцы «Наименование», «№ внутренний», «Дата» и «Ответственный» обязательны. Внутренний номер используется
как уникальный идентификатор записи, поэтому при повторном импорте строки с тем же номером обновляют существующий договор.
Внешний номер можно оставить пустым — он перезаписывается только если указан в файле. Дату следует задавать в формате
`ГГГГ-ММ-ДД` или `ДД.ММ.ГГГГ` (например, `2024-05-31` или `31.05.2024`).

Столбец «Контрагент» необязателен. Если имя задано, оно должно в точности соответствовать существующему контрагенту; в
противном случае импорт остановится с ошибкой. Пустое значение снимает привязку договора к контрагенту.

Пример файла импорта договоров:

| Наименование                  | № внутренний | № внешний | Дата       | Ответственный     | Контрагент     |
|-------------------------------|--------------|-----------|------------|-------------------|----------------|
| Сервисное обслуживание ИТ     | ДГ-001       | 45/2024   | 2024-05-01 | Иванов И.И.       | ООО «Альфа»    |
| Поставка офисного оборудования | ДГ-002       |           | 15.05.2024 | Петров П.П.       |                |

## Импорт способов закупки из Excel
Раздел «Справочники» → «Способ закупки» также поддерживает загрузку данных из файлов `.xlsx`.
Ограничения аналогичны остальным справочникам: размер файла не более 10 МБ, максимум 50 000 строк данных,
обрабатывается только первый лист книги.

Первая строка листа должна содержать единственный столбец «Наименование». Каждая последующая строка описывает отдельный
способ закупки. Названия сравниваются без учёта регистра и пробелов по краям: при повторной загрузке строка с тем же
именем обновляет существующую запись. Пустые строки игнорируются.

Пример таблицы для импорта способов закупки:
